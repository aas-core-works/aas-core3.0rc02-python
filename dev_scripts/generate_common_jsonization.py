...
class ExampleKind(enum.Enum):
    ...
    #: Complete instance with all the properties set
    COMPLETE = 0
...
def _generate_load_expected(
    ...
    """
    Generate the function to load the complete example from a container.
    ...
    if example_kind is ExampleKind.COMPLETE:
        file_name_literal = Stripped(
            aas_core_codegen.python.common.string_literal("complete.json")
        )
    ...
    if example_kind is ExampleKind.COMPLETE:
        function_name = Stripped(
            aas_core_codegen.python.naming.function_name(
                aas_core_codegen.common.Identifier(f"load_complete_{cls.name}")
            )
        )
    elif example_kind is ExampleKind.MINIMAL:
        function_name = Stripped(
...
{II}aas_common.assert_never(expected)"""
    ...
    for our_type in symbol_table.our_types:
        ...
        blocks.append(
            _generate_load_expected(
                ...
                example_kind=ExampleKind.COMPLETE,
            )
        )
    ...
    target_pth = repo_root / "tests/common_jsonization.py"
